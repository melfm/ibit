...
AGENT_TRAIN_FORMAT = {
    ...
    'deepmdp': [('batch_reward', 'BR', 'float'),
                ('actor_loss', 'ALOSS', 'float'),
                ('critic_loss', 'CLOSS', 'float'),
                ('alpha_loss', 'TLOSS', 'float'),
                ('alpha_value', 'TVAL', 'float'),
                ('actor_entropy', 'AENT', 'float')],
    'dbc': [('batch_reward', 'BR', 'float'), ('actor_loss', 'ALOSS', 'float'),
            ('critic_loss', 'CLOSS', 'float'),
            ('alpha_loss', 'TLOSS', 'float'), ('alpha_value', 'TVAL', 'float'),
            ('actor_entropy', 'AENT', 'float')],
}
...
class Logger(object):
    def __init__(self,
        ...
        self._eval_us_mg = MetersGroup(os.path.join(log_dir, 'eval_unseen'),
                                    formating=COMMON_EVAL_FORMAT)
        self._eval_sa_mg = MetersGroup(os.path.join(log_dir, 'eval_standalone'),
                                    formating=COMMON_EVAL_FORMAT)
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        ...
        if key.startswith('train'):
            ...
            mg = self._eval_us_mg
        elif key.startswith('eval_standalone'):
            mg = self._eval_sa_mg
    ...
    def dump(self, step, save=True, ty=None):
        ...
        if ty is None:
            ...
            self._eval_sa_mg.dump(step, 'eval_standalone', save)
        elif ty == 'eval_seen':
            self._eval_s_mg.dump(step, 'eval_seen', save)
        elif ty == 'eval_unseen':
            self._eval_us_mg.dump(step, 'eval_unseen', save)
        elif ty == 'eval_standalone':
            self._eval_sa_mg.dump(step, 'eval_standalone', save)
        elif ty == 'train':
            self._train_mg.dump(step, 'train', save)
        else:
